...
checkSimInput::checkSimInput(TTree *tree, std::string iTempFileName,std::string oTempFileName) : fChain(0) 
{
...
void checkSimInput::Loop()
{
   ...
   // for (Long64_t jentry=0; jentry < nentries;++jentry) {
      ...
      // for(size_t i = 0; i < m_SimTrackerHitsKuma.at(11).getSize(); ++i){
      //    Double_t time = m_SimTrackerHitsKuma.at(11).getTime(i);
      ...
      for(int iDet = 0; iDet < 17; ++iDet){
         ...
         Double_t minTimeInAnEvent = 9999.; // Initialize to a large value
         Int_t minTimeInAnEventID = -1; // Initialize to a large value
         Double_t timeChi2 = 0.; // Initialize to a small value                               

         Int_t numberOfHits = m_SimTrackerHitsKuma.at(iDet).getSize();
         if(numberOfHits == 0) continue; // Skip if no hits in the detector
         Int_t numOfHitsInTimeWindow = 0;
         for (int iHit = 0; iHit < numberOfHits; ++iHit) {            
            Double_t time = m_SimTrackerHitsKuma.at(iDet).getTime(iHit);            
            timeChi2 += time * time; // Sum of squares for time

            if (time < m_timeWindows[iDet]) numOfHitsInTimeWindow++;

            // Optionally, fill histograms for SiBarrelHits (index 11)
            if (time < minTimeInAnEvent) {
               minTimeInAnEvent = time;
               minTimeInAnEventID = iHit; // Store the index of the hit with minimum time
            }
            m_hTimeDispersionDetComp->Fill(iDet + 0.5, time);

            Double_t energy = m_SimTrackerHitsKuma.at(iDet).getEDep(iHit);
            m_hEnergyDist[iDet]->Fill(energy);
            m_hHitTimeVsDepE[iDet]->Fill(time, energy);
            sumEnergyInAnEvent += energy;
            energyChi2 += energy * energy; // Sum of squares for energy
            ...
            m_hDetectorHitsPosiZXY[iDet]->Fill(hitZ, hitX, hitY);
            m_hDetectorHitsPosiZR[iDet]->Fill(hitZ, hitR);
            m_hDetectorHitsPosiXY[iDet]->Fill(hitX, hitY);

            m_hZVsHitTimeInclusive->Fill(hitZ, time+(2000*jentry));
            m_hXYVsHitTime_DIRCBar->Fill(hitX, hitY, time);

            if(!bTargetEV) continue;
            m_hEventDisplayZXY_Det->Fill(hitZ, hitX, hitY);
            m_hEventDisplayZR_Det->Fill(hitZ, hitR);
         }// == end of loop over hits in a detector

         m_hNumOfHitsInTimeWindow[iDet]->Fill(numOfHitsInTimeWindow);
         
         minTimeArray[iDet] = minTimeInAnEvent; // Store the minimum time for each detector
         m_hMinTimeHit[iDet]->Fill(minTimeInAnEvent);
         m_hHitMinTimeDetComp->Fill(iDet + 0.5, minTimeInAnEvent);
         
         timeChi2 = TMath::Sqrt(timeChi2) / numberOfHits;
         m_hTimeDispersionOfAnEvent[iDet]->Fill(timeChi2);
         energyChi2 = TMath::Sqrt(energyChi2) / numberOfHits;
         m_hEnergyDispersionOfAnEvent[iDet]->Fill(energyChi2);
               
         for (int iHit = 0; iHit < numberOfHits; ++iHit) {
            if(minTimeInAnEventID == iHit) continue; // Skip the hit with minimum time
            Double_t time = m_SimTrackerHitsKuma.at(iDet).getTime(iHit);
            Double_t diffTime = time - minTimeInAnEvent;
            m_hMinusMinTimeDist[iDet]->Fill(diffTime);
            m_hMinusMinTimeDistWideRange[iDet]->Fill(diffTime);
            ...
            m_hRVsHitTimeInclusive->Fill(r, time);
            m_hRVsHitTime[iDet]->Fill(r, diffTime);

            Double_t calibratedTime = HitTimeCalibrationByR(time, r);
            m_hCalibratedTimeHit[iDet]->Fill(calibratedTime);
         }

         for(int jDet = 0; jDet < 17; ++jDet){
            Int_t jNumberOfHits = m_SimTrackerHitsKuma.at(jDet).getSize();
            
            if(jDet == iDet) continue; // Skip if the same detector
            if(jNumberOfHits == 0) continue; // Skip if no hits in the detector
            // if(!bTargetEV && iDet==16 && jDet == 11) m_vTargetEvents.push_back(m_pubEvNum);
            m_hDetHitsCombination->Fill(iDet + 0.5, jDet + 0.5, jNumberOfHits);
            m_hDetHitsEffCombination->Fill(iDet + 0.5, jDet + 0.5, 1);
         } // == end of loop over detectors

         numOfEvents[iDet]++;
      } // == end of loop over detectors

      // Fill histograms for relative hit time for Vertex Barrel
      for(int iDet = 0; iDet < 16; ++iDet){
         if(minTimeArray[iDet] < 0) continue; // Skip if no hits in the detector
         Double_t hitRelativeTime = minTimeArray[iDet] - minTimeArray[16];
         m_hRelativeHitTimeForVtxBarrel->Fill(iDet + 0.5, hitRelativeTime);
         m_hRelativeHitTimeForVtxBarrelWideRange->Fill(iDet + 0.5, hitRelativeTime);
      }
      ...
      for (int iP = 0; iP < MCParticles_; ++iP) {
         ...
         // Fill histograms for minimum time in each detectorMCParticles_vertex_x
         Double_t simPIX = MCParticles_vertex_x[iP];
         Double_t simPIY = MCParticles_vertex_y[iP];
         Double_t simPIZ = MCParticles_vertex_z[iP];
         Double_t simPFX = MCParticles_endpoint_x[iP];
         Double_t simPFY = MCParticles_endpoint_y[iP];
         Double_t simPFZ = MCParticles_endpoint_z[iP];
         
         m_lEventDisplay_SimP->SetPoint(0, simPIX, simPIY, simPIZ);
         m_lEventDisplay_SimP->SetPoint(1, simPFX, simPFY, simPFZ);
      }
   ...
   Double_t timeWindow90Percent[17] = {0.0};
   for (int iDet = 0; iDet < 17; ++iDet) {
      for (int jDet = 0; jDet < 17; ++jDet) {
         Double_t numOfEventsInCombination = m_hDetHitsEffCombination->GetBinContent(iDet+1, jDet+1);
         if(numOfEvents[iDet]==0) m_hDetHitsEffCombination->SetBinContent(iDet + 1, jDet + 1, 0.);
         else {
            Double_t numOfEventsInCombinationScaled = numOfEventsInCombination / numOfEvents[iDet];
            m_hDetHitsEffCombination->SetBinContent(iDet + 1, jDet + 1, numOfEventsInCombinationScaled);
         }
      }
      m_hEventNumberForEachDetector->SetBinContent(iDet + 1, numOfEvents[iDet]);
      m_hEventNumberRatioForEachDetector->SetBinContent(iDet + 1, (1.0*numOfEvents[iDet]) / (1.0*numOfEvents[17]));

      Int_t numOfBins = m_hCalibratedTimeHit[iDet]->GetNbinsX();
      Int_t tempNumOfHits90Percent = 0;
      for(Int_t iBin = 1; iBin <= numOfBins; ++iBin) {
         Double_t binContent = m_hCalibratedTimeHit[iDet]->GetBinContent(iBin);
         tempNumOfHits90Percent += binContent;
         if(tempNumOfHits90Percent > 0.9 * m_hCalibratedTimeHit[iDet]->Integral()) {
            timeWindow90Percent[iDet] = m_hCalibratedTimeHit[iDet]->GetBinCenter(iBin);
            break;
         }

      }
      std::cout << " timeWindow90Percent["<< iDet + 1 <<"] = " << timeWindow90Percent[iDet] << std::endl;
      m_hCalibratedTimeWindow90Percent->Fill(iDet + 1, timeWindow90Percent[iDet]);
   }
   m_hEventNumberForEachDetector->SetBinContent(18, numOfEvents[17]);
   m_hEventNumberRatioForEachDetector->SetBinContent(18, 1.0);
   ...
   std::cout << "Double_t m_timeWindows[17] = { ";   
...
void checkSimInput::HistInit(){
   ...
   m_hEventNumberForEachDetector = new TH1D(
      "m_hEventNumberForEachDetector",
      "m_hEventNumberForEachDetector; sub-detector; count",
      18, 0.5, 18
   );

   m_hEventNumberRatioForEachDetector = new TH1D(
      "m_hEventNumberRatioForEachDetector",
      "m_hEventNumberRatioForEachDetector; sub-detector; count",
      18, 0.5, 18
   );
   ...
   m_hDetHitsEffCombination = new TH2D(
      "m_hDetHitsEffCombination",
      "m_hDetHitsEffCombination; trigger; count",
      17, 0.5, 17, 17, 0.5, 17
   );


   m_hRelativeHitTimeForVtxBarrel = new TH2D(
      "m_hRelativeHitTimeForVtxBarrel",
      "m_hRelativeHitTimeForVtxBarrel; sub-detector; #it{t}_{det} - #it{t}_{VtxBarrel} [ns]",
      17, 0.5, 17, 2000, -1, 1
   );
   m_hRelativeHitTimeForVtxBarrelWideRange = new TH2D(
      "m_hRelativeHitTimeForVtxBarrelWideRange",
      "m_hRelativeHitTimeForVtxBarrelWideRange; sub-detector; #it{t}_{det} - #it{t}_{VtxBarrel} [ns]",
      17, 0.5, 17, 800, -30, 50
   );
   ...
   m_hZVsHitTimeInclusive = new TH2D(
      "m_hZVsHitTimeInclusive",
   ...
   m_hCalibratedTimeWindow90Percent = new TH1D(
      ...
      "m_hCalibratedTimeWindow90Percent; ; time [ns]",
      17, 0.5, 17
   );


   m_hXYVsHitTime_DIRCBar = new TProfile2D(
      "m_hXYVsHitTime_DIRCBar",
      "m_hXYVsHitTime_DIRCBar; x [mm]; y [mm]; time [ns]",
      100, -2000, 2000, 100, -2000, 2000
   );

   m_hHitMinTimeDetComp = new TH2D(
      "m_hHitMinTimeDetComp",
      "hHitMinTimeDetComp; sub-detector; #it{t}_{min} [ns]",
      17, 0.5, 17, 2000, 0, 200
   );
   m_hTimeDispersionDetComp = new TH2D(
   ...
   m_hEventNumberForEachDetector->GetXaxis()->SetBinLabel(18, "AllEvents");
   m_hEventNumberRatioForEachDetector->GetXaxis()->SetBinLabel(18, "AllEvents");
   for (int i = 0; i < 17; ++i) {
      m_hEventNumberForEachDetector->GetXaxis()->SetBinLabel(i + 1, m_simTrackHitNames[i].Data());
      m_hEventNumberRatioForEachDetector->GetXaxis()->SetBinLabel(i + 1, m_simTrackHitNames[i].Data());

      m_hDetHitsCombination->GetXaxis()->SetBinLabel(i + 1, m_simTrackHitNames[i].Data());
      m_hDetHitsCombination->GetYaxis()->SetBinLabel(i + 1, m_simTrackHitNames[i].Data());

      m_hDetHitsEffCombination->GetXaxis()->SetBinLabel(i + 1, m_simTrackHitNames[i].Data());
      m_hDetHitsEffCombination->GetYaxis()->SetBinLabel(i + 1, m_simTrackHitNames[i].Data());

      m_hHitMinTimeDetComp->GetXaxis()->SetBinLabel(i + 1, m_simTrackHitNames[i].Data());
      m_hHitMinTimeDetComp->GetXaxis()->LabelsOption("d");
      
      m_hTimeDispersionDetComp->GetXaxis()->SetBinLabel(i + 1, m_simTrackHitNames[i].Data());
      m_hTimeDispersionDetComp->GetXaxis()->LabelsOption("d");

      m_hRelativeHitTimeForVtxBarrel->GetXaxis()->SetBinLabel(i + 1, m_simTrackHitNames[i].Data());
      m_hRelativeHitTimeForVtxBarrelWideRange->GetXaxis()->SetBinLabel(i + 1, m_simTrackHitNames[i].Data());

      m_hCalibratedTimeWindow90Percent->GetXaxis()->SetBinLabel(i + 1, m_simTrackHitNames[i].Data());


      m_hMinTimeHit[i] = new TH1D(
         Form("m_hMinTimeHit_%s", m_simTrackHitNames[i].Data()),
         Form("hMinTimeHit %s; time [ns]; count", m_simTrackHitNames[i].Data()),
         1000, 0, 10000
      );


      m_hTimeDispersionOfAnEvent[i] = new TH1D(
         Form("m_hTimeDispersionOfAnEvent_%s", m_simTrackHitNames[i].Data()),
         Form("hTimeDispersionOfAnEvent %s; time [ns]; count", m_simTrackHitNames[i].Data()),
         1000, 0, 10000
      );

      m_hMinusMinTimeDist[i] = new TH1D(
         Form("m_hMinusMinTimeDist_%s", m_simTrackHitNames[i].Data()),
         Form("hMinusMinTimeDist %s; #it{t}_{i} - #it{t}_{min} [ns]; count", m_simTrackHitNames[i].Data()),
         1000, 0, 5
      );

      m_hMinusMinTimeDistWideRange[i] = new TH1D(
         Form("m_hMinusMinTimeDistWideRange_%s", m_simTrackHitNames[i].Data()),
         Form("hMinusMinTimeDistWideRange %s; #it{t}_{i} - #it{t}_{min} [ns]; count", m_simTrackHitNames[i].Data()),
         2000, 0, 200
      );

      m_hRVsHitTime[i] = new TH2D(
         Form("m_hRVsHitTime_%s", m_simTrackHitNames[i].Data()),
         Form("hRVsHitTime %s; r [mm]; time [ns]", m_simTrackHitNames[i].Data()),
         1000, 0, 1000, 2000, 0, 200
      );

      m_hHitTimeVsDepE[i] = new TH2D(
         Form("m_hHitTimeVsDepE_%s", m_simTrackHitNames[i].Data()),
         Form("hHitTimeVsDepE %s; time [ns]; depE [MeV]", m_simTrackHitNames[i].Data()),
         2000, 0, 200, 1000, 0, 10
      );

      m_hCalibratedTimeHit[i] = new TH1D(
         Form("m_hCalibratedTimeHit_%s", m_simTrackHitNames[i].Data()),
         Form("m_hCalibratedTimeHit %s; time [ns]; count", m_simTrackHitNames[i].Data()),
         2000, 0, 200
      );
      m_hNumOfHitsInTimeWindow[i] = new TH1D(
         Form("m_hNumOfHitsInTimeWindow_%s", m_simTrackHitNames[i].Data()),
         Form("m_hNumOfHitsInTimeWindow %s; # of hits / time window; count", m_simTrackHitNames[i].Data()),
         100, 0, 100
      );


      m_hEnergyDist[i] = new TH1D(
         Form("m_hEnergyDist_%s", m_simTrackHitNames[i].Data()),
         Form("hEnergyDist %s; energy [MeV]; count", m_simTrackHitNames[i].Data()),
         1000, 0, 10
      );

      m_hEnergyDispersionOfAnEvent[i] = new TH1D(
         Form("m_hEnergyDispersionOfAnEvent_%s", m_simTrackHitNames[i].Data()),
         Form("hEnergyDispersionOfAnEvent %s; energy [MeV]; count", m_simTrackHitNames[i].Data()),
         1000, -10, 10
      );


      m_hDetectorHitsPosiZXY[i] = new TH3D(
         Form("m_hDetectorHitsPosiZXY%s", m_simTrackHitNames[i].Data()),
         Form("m_hDetectorHitsPosiZXY %s; z [mm]; x [mm]; y [mm]", m_simTrackHitNames[i].Data()),
         20, -10000, 10000, 100, -2500, 2500, 100, -2500, 2500
      );
      m_hDetectorHitsPosiZXY[i]->SetLineColor(m_myHistColors[i]);
      
      m_hDetectorHitsPosiZR[i] = new TH2D(
         Form("m_hDetectorHitsPosiZR%s", m_simTrackHitNames[i].Data()),
         Form("m_hDetectorHitsPosiZR %s; z [mm]; r [mm]", m_simTrackHitNames[i].Data()),
         3000, -15000, 15000, 5000, 0, 2500
      );
      m_hDetectorHitsPosiZR[i]->SetLineColor(m_myHistColors[i]);
            
      m_hDetectorHitsPosiXY[i] = new TH2D(
         Form("m_hDetectorHitsPosiXY%s", m_simTrackHitNames[i].Data()),
         Form("m_hDetectorHitsPosiXY %s; x [mm]; y [mm]", m_simTrackHitNames[i].Data()),
        5000, -1000, 1000, 5000, -1000, 1000
      );
   ...
   // m_hEventDisplay_SimP = new TH3D(
   //    "m_hEventDisplay_SimP",
   //    "Event Display; z [mm]; x [mm]; y [mm]",
   //    100, -10000, 10000, 100, -2500, 2500, 100, -2500, 2500
   // );
}
...
void checkSimInput::ResetValuesForEachEvent(){

   m_SimTrackerHitsKuma.clear();
   m_SimTrackerHitsKuma.shrink_to_fit();
}

void checkSimInput::WriteHists(){
   oFile->cd();

   m_hEventNumberForEachDetector->Write();
   m_hEventNumberRatioForEachDetector->Write();

   m_hDetHitsCombination->Write();
   m_hDetHitsEffCombination->Write();
   ...
   for (int i = 0; i < 17; ++i) {
      ...
      m_hTimeDispersionOfAnEvent[i]->Write();
      m_hMinusMinTimeDist[i]->Write();
      m_hMinusMinTimeDistWideRange[i]->Write();
      m_hRVsHitTime[i]->Write();
      m_hHitTimeVsDepE[i]->Write();

      m_hCalibratedTimeHit[i]->Write();
      m_hNumOfHitsInTimeWindow[i]->Write();

      m_hEnergyDist[i]->Write();
      m_hEnergyDispersionOfAnEvent[i]->Write();

      m_hDetectorHitsPosiZXY[i]->Write();
      m_hDetectorHitsPosiZR[i]->Write();
      m_hDetectorHitsPosiXY[i]->Write();
   }
...
Int_t checkSimInput::GetEntry(Long64_t entry)
{
...
Long64_t checkSimInput::LoadTree(Long64_t entry)
{
...
void checkSimInput::Init(TTree *tree)
{
...
void checkSimInput::Show(Long64_t entry)
{
...
Int_t checkSimInput::Cut(Long64_t entry)
{

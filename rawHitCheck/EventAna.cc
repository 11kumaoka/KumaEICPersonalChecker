...
void EventAna::EventLoop() {
    ...
    for (unsigned iEvent = 0; iEvent < nEvents; ++iEvent) {
        ...
        FillEachSubDetDepE();
        FillEachSubDetDepEWEachBKG(frame);
...
void EventAna::FillEachSubDetDepEWEachBKG(const podio::Frame& frame){
    const auto& mcP = frame.get<edm4hep::MCParticleCollection>("MCParticles");

    Int_t tempDetID = 0;
    for (const auto collectionName : InputDataConfig::kTrkCollections) {
        Int_t repDetId = 6;
        ...
        const auto& hits = frame.get<edm4hep::SimTrackerHitCollection>(std::string(collectionName));
        
        for (const auto& hit : hits) {
            const auto pos = hit.getPosition();
            auto relMcP = hit.getParticle();
            auto relMcPId = relMcP.getObjectID();
            Int_t relMcPIdGenId = mcP.at(relMcPId.index).getGeneratorStatus();

            Double_t depE = hit.getEDep() * 1000; // Convert GeV to MeV
            Int_t pKindId = relMcPIdGenId/1000 -1;
            if(pKindId < 0) pKindId = 0;
            m_hDetEDep[repDetId][pKindId]->Fill(depE);

            // std::cout << std::string(collectionName) <<  ":repDetId" << repDetId << ", genID" << relMcPIdGenId << ": pKindId" << pKindId << std::endl;
...
void EventAna::OFileInit() {
    ...
    for(size_t iTrkDet = 0; iTrkDet < 7; iTrkDet++){
        for(size_t iPKind = 0; iPKind < 6; iPKind++){
            TString histName = TString::Format("m_hMCDetE_%s_%s",m_trkShortDetName[iTrkDet].Data(),m_physKindShortName[iPKind].Data());
            TString histTitle = TString::Format("%s;eDep [MeV];count",histName.Data());
            m_hDetEDep[iTrkDet][iPKind] = new TH1D(histName.Data(), histTitle.Data(), 100, 0, 0.50);
        }
    }
...
void EventAna::EditHists() {
    for(size_t iTrkDet = 0; iTrkDet < 7; iTrkDet++){
        for(size_t iPKind = 0; iPKind < 6; iPKind++){
            m_hDetEDep[iTrkDet][iPKind]->SetLineColor(m_BKGColors[iPKind]);
            m_hDetEDep[iTrkDet][iPKind]->SetLineWidth(5);
        }
    }
}
...
void EventAna::OFileWrite() {
    ...
    for(size_t iTrkDet = 0; iTrkDet < 7; iTrkDet++){
        for(size_t iPKind = 0; iPKind < 6; iPKind++){
            m_hDetEDep[iTrkDet][iPKind]->Write();
        }
    }

...
int main() {
    ...
    reader.openFile("/Users/tkumaoka/eic/inputFiles/pythia500kHz_orig.root");
    ...
    for (size_t iOrigEvent = 0; iOrigEvent < numEvents; ++iOrigEvent) {
        // Read input frame
        auto frameData = reader.readEntry("events", iOrigEvent);
        ...
        // Add dummy frames
        for (int iAddDummy = 0; iAddDummy < 4; ++iAddDummy) {
            podio::Frame empty_frame;
            

            // edm4hep::MutableEventHeader event_header_ts;
            // event_header_ts.setRunNumber(iAddDummy + 100*iOrigEvent);
            // event_header_ts.setEventNumber(iAddDummy + 100*iOrigEvent);
            // event_header_ts.setTimeStamp(0);
            // event_header_ts.setWeight(0); // Just a dummy weight for now
            ...
            for (const auto& name : m_simtrackerhit_collection_names) {
                ...
                emptyHit.setTime(9999999999999.0);
